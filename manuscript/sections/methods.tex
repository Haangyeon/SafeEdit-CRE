...
180-parent subset, and the nine-criterion benchmark used a fixed 90-parent confirmatory
subset after six nominal parents were found to overlap with an earlier exploratory
benchmark and were excluded by identifier without reference to outcomes.
...
training records to provide held-out evaluation at different stages. The reviewer
...
heads~\citep{He2016,Lakshminarayanan2017}. The use of seeded ensembles and predictive
variance follows established neural-network uncertainty strategies~\citep{Gal2016,Blundell2015,Lakshminarayanan2017}.
Each architecture used three fixed seeds:
...
dropout 0.25~\citep{Srivastava2014}, and gradient clipping at 5.0. The loss combined heteroscedastic Gaussian
...
The nine-criterion benchmark (G4) used 90 test-split CRE parents with no overlap to
development or primary-evaluation parents (six nominal parents were excluded
after identifier-level overlap with an exploratory benchmark was discovered, without
reference to outcomes). For each method--parent--target--budget
...
The 3,240 nine-criterion-benchmark candidates were organized into three computational
...
in the frozen release bundle. Statistical values and figures were
...
sequence tables, and retained model checkpoints are included in the release bundle.
